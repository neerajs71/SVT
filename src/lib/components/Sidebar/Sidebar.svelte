<script>
  ...
  import { defaultSidebarTab } from '$lib/utils/device.js';

  let { open = $bindable(false) } = $props();

  // ── Sidebar tab: 'local' | 'remote' | 'working' ──────────────────────────
  // Defaults to 'working' on iOS/mobile (no File System Access API)
  let sidebarTab = $state(defaultSidebarTab(datasourceStore.mode));
